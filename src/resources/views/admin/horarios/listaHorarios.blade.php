...
<input type="search" id="horarios-search" class="form-control admin-search-input" placeholder="Pesquisar horários" aria-label="Pesquisar horários" />
</div>
<select id="horarios-status-filter" class="form-select form-select-sm w-auto" aria-label="Filtrar por status">
...
<td>{{ $horario->telefone_horarios }}</td>
<td>{{ $horario->formato_horarios }}</td>
<td>{{ $horario->regiao_horarios }}</td>
<td>{{ $horario->horario_horarios }}</td>
<td>
...
<div class="float-start pt-1 fs-7 text-body-secondary">Total de horários: <strong>{{ $horarios->count() }}</strong>
</div>
